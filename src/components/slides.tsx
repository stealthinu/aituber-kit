...
const stopRecordingDelayMs = (() => {
  ...
  if (!Number.isFinite(parsedSeconds) || parsedSeconds < 0) {
    console.warn(
      'NEXT_PUBLIC_OBS_STOP_DELAY_SECONDS は無効な値のため 0 に設定されました'
    )
    return 0
  }
...
const Slides: React.FC<SlidesProps> = ({ markdown }) => {
  ...
  const connectToOBS = useCallback(async () => {
    ...
    try {
      ...
      obs.on('RecordStateChanged', (event: { outputActive: boolean }) => {
        console.log(
          `OBSの録画状態が変更されました: ${event.outputActive ? '録画中' : '録画停止'}`
        )
  ...
  const checkAllVideosEnded = useCallback(() => {
    ...
    // すべてのビデオが終了しているかチェック
    return currentVideosRef.current.every(
      (video) =>
    ...
    (slideIndex: number) => {
      const getCurrentLines = () => {
        try {
          ...
          console.error(
            `スライド「${selectedSlideDocs}」のスクリプト読み込みに失敗しました:`,
            error
          )
          return ''
        }
  ...
  useEffect(() => {
    ...
    slides.forEach((slide, i) => {
      if (i === currentSlide) {
        ...
        if (videos.length > 0) {
          console.log(
            `スライド${currentSlide}には${videos.length}個の動画があります`
          )
  ...
  useEffect(() => {
    const convertMarkdown = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `スライドの変換に失敗しました: ${response.statusText}`
          )
        }
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        'スライドの準備が完了し、自動再生モードが有効なため、再生を開始します'
      )
      // 再生開始フラグを設定
  ...
  useEffect(() => {
    ...
    if (
      ...
      const timerId = setTimeout(() => {
        ...
        if (
          slideStore.getState().isPlaying &&
          currentSlide < slideCount - 1 &&
          homeStore.getState().chatProcessingCount === 0 &&
          !videoPlaying
        ) {
          console.log(
            '音声ナレーションと動画の両方が終了したため、次のスライドへ進みます'
          )
          nextSlide()
        }
      }, 500)
